...
<h1 align="center">@fengluo/vant-element</h1>
...
<p align="center">
  <img src="https://img.shields.io/npm/v/@fengluo/vant-element" alt="npm">
  <img src="https://img.shields.io/npm/l/@fengluo/vant-element" alt="license">
</p>

## 背景

桌面端使用 Element-UI，移动端需要重新写一套 Vant 代码？不需要。

`@fengluo/vant-element` 是一个 Vue 2 插件，全局注册了 `el-` 前缀的组件和服务，内部调用 Vant 组件并自动完成 API 映射。你只需要写 Element-UI 的代码，就能在移动端渲染出 Vant 的 UI。
...
完整的使用文档请查看 [在线文档](https://vant-ui.github.io/vant/v2/#/zh-CN/quickstart)。
...
Vue.use(VantElement, options)
```

| 参数 | 说明 | 类型 | 默认值 |
|------|------|------|--------|
| `components` | 需要注册的组件白名单，不传则注册全部 | `string[]` | `-` |
...
本项目为 npm workspaces monorepo，包含两个包：

```
vant-element/
├── packages/
│   └── vant-element/         # 库源码 @fengluo/vant-element
│       ├── src/
│       │   ├── index.js      # 插件入口
│       │   ├── utils.js      # 图标映射工具
│       │   ├── components/   # 65 个组件
